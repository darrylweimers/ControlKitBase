...
public class MenuViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    public init(images: [UIImage],
        ...
        self.itemManager = ItemManager(numberOfItems: _menuImages.count, numberOfVisibleItems: estimatedNumberOfVisibleItems)
    ...
    private func setupViews(superview: UIView) {
        ...
        NSLayoutConstraint.activate([
            collectionView.centerXAnchor.constraint(equalTo: superview.centerXAnchor),
            collectionView.widthAnchor.constraint(equalTo: superview.widthAnchor),
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.image = _menuImages[normalizedIndexPath.row]
    ...
    public func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        let bounds = collectionView.bounds
        let xPosition = collectionView.contentOffset.x + bounds.size.width / 2.0
        ...
        let itemDiameter = self.getItemDiameter(collectionView: collectionView, itemScaleRatio: itemDiameterScaleRatio)
        let adjustedPoint = CGPoint(x: xPosition - itemDiameter / 2, y: yPosition)
        if let indexPath = collectionView.indexPathForItem(at: adjustedPoint) {
            selectCell(for: indexPath, animated: true)
            return
        }
    }
